...
# En Vercel los archivos están en /var/task, localmente en la carpeta del proyecto
...
def compute():
    ...
    if sensor_type in ("polar_h10", "rr_upload"):
        ...
        result = compute_hrv_from_ppg(np.array(ppg, dtype=float), sr, duration_minutes=duration_minutes)
    else:
...
def save():
    ...
    if sb:
        try:
            ...
            return app.response_class(json.dumps({"ok": False, "error": str(e)}), status=500, headers=CORS)
    else:
...
def history():
    ...
    sb = _sb()
    if not sb:
        return app.response_class(json.dumps({"data": []}), status=200, headers=CORS)
    try:
        ...
        return app.response_class(json.dumps({"error": str(e)}), status=500, headers=CORS)
