/* HBA v3 — main.js · Pantallas separadas */
"use strict";
...
const G = {
  sensor: "camera_ppg", durationMin: 3, totalSec: 180,
  running: false, ppgBuf: [], rrBuf: [], sigBuf: [],
  elapsed: 0, lastMetrics: null,
  timerH: null, rafH: null, chartLast: 0,
  bleDevice: null, stream: null, frameCtx: null, motionH: null,
};
...
function buildChart() {
  chart = new Chart($("signalChart").getContext("2d"), {
    type: "line",
    data: {
...
function pushSig(v) {
  G.sigBuf.push(v);
  if (G.sigBuf.length > WIN * 3) G.sigBuf.shift();
}

function renderChart(now) {
  G.rafH = requestAnimationFrame(renderChart);
  if (!G.running || now - G.chartLast < 1000 / FPS) return;
  G.chartLast = now;
  const buf = G.sigBuf;
  const data = buf.length >= WIN ? buf.slice(-WIN) : [...Array(WIN - buf.length).fill(null), ...buf];
...
function startTimer() {
  ...
  G.timerH = setInterval(() => {
    ...
    if (G.elapsed >= G.totalSec) stopSession(true);
  }, 1000);
}

function stopTimer() {
  if (G.timerH) { clearInterval(G.timerH); G.timerH = null; }
}

/* ── Countdown ───────────────────────────────── */
function countdown(title, text, hint, secs) {
...
function setStatus(text, cls = "") {
  ...
  const dot = $("statusDot");
  dot.className = "pill-dot" + (cls ? " " + cls : "");
}

function setQuality(q) {
  const fill = $("qualityFill"), txt = $("qualityText");
  if (q == null) { fill.style.width = "0%"; txt.textContent = "—"; return; }
  fill.style.width = `${q}%`;
  txt.textContent  = `${Math.round(q)}%`;
  fill.style.background = q >= 70 ? "var(--s-opt)" : q >= 40 ? "var(--s-com)" : "var(--s-cri)";
}
...
function setBtns(running) {
  ...
  $("btnSave2").disabled = !G.lastMetrics;
}

/* ── Selector sensor ─────────────────────────── */
const SENSOR_META = {
  camera_ppg   : { label: "Cámara PPG",  dur1: false, torch: true,  camera: true,  guide: "Apoyá el codo. Cubrí completamente el lente y el flash con el dedo índice. Respirá con normalidad." },
  face_rppg    : { label: "rPPG Facial", dur1: true,  torch: false, camera: true,  guide: "Sentate frente a la cámara con luz uniforme. No te muevas durante el test." },
  vibration_scg: { label: "SCG",         dur1: true,  torch: false, camera: false, guide: "Acostáte. Colocá el celular centrado sobre el esternón y otorgá permiso de movimiento." },
...
document.querySelectorAll(".dur-btn").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    G.durationMin = parseInt(btn.dataset.min);
    G.totalSec    = G.durationMin * 60;
    $("durHint").textContent = G.durationMin < 5
      ? `Test ${G.durationMin} min: LF/HF orientativo. Para máxima precisión: 5 min.`
      : "5 min: análisis espectral LF/HF más estable.";
  });
});
...
async function startCamera() {
  ...
  G.stream = await navigator.mediaDevices.getUserMedia({
    video: { facingMode: isFace ? "user" : "environment", width: { ideal: 160 }, height: { ideal: 120 }, frameRate: { ideal: PPG_SR } }
  });
  const vid = $("video");
  vid.srcObject = G.stream;
  await vid.play();

  if (!isFace && $("torchToggle").checked) {
    try { await G.stream.getVideoTracks()[0].applyConstraints({ advanced: [{ torch: true }] }); } catch (_) {}
  }
  ...
  function capture(ts) {
    ...
    if (ts - last >= interval) {
      last = ts;
      G.frameCtx.drawImage(vid, 0, 0, 4, 4);
      const px = G.frameCtx.getImageData(0, 0, 4, 4).data;
      let val = 0;
      if (isFace) { for (let i = 1; i < px.length; i += 4) val += px[i]; }
      else        { for (let i = 0; i < px.length; i += 4) val += px[i]; }
      val /= (px.length / 4);
      G.ppgBuf.push(val);
      pushSig(val);
    }
...
function stopCamera() {
  if (G.stream) {
    G.stream.getTracks().forEach(t => {
      try { t.applyConstraints({ advanced: [{ torch: false }] }); } catch (_) {}
      t.stop();
    });
    G.stream = null;
  }
...
function startVibration() {
  ...
  const bind = () => {
    ...
    window.addEventListener("devicemotion", G.motionH, { passive: true });
  };
...
function stopVibration() {
  if (G.motionH) { window.removeEventListener("devicemotion", G.motionH); G.motionH = null; }
}
  ...
  ch.addEventListener("characteristicvaluechanged", e => {
    ...
    if (hasRR) {
      while (offset + 1 < dv.byteLength) {
        ...
        offset += 2;
        if (rr > 300 && rr < 2200) { G.rrBuf.push(rr); pushSig(rr); }
      }
    }
  });
...
function stopPolarH10() {
  try { if (G.bleDevice?.gatt?.connected) G.bleDevice.gatt.disconnect(); } catch (_) {}
  G.bleDevice = null;
}
...
async function loadRRFile() {
  ...
  if (file.name.toLowerCase().endsWith(".json")) {
    ...
    rrs = Array.isArray(obj) ? obj : (obj.rri_ms || obj.rr || Object.values(obj).flat());
    rrs = rrs.map(Number);
  } else {
    const lines  = text.trim().split(/\r?\n/);
    const header = lines[0].toLowerCase().split(/[,;\t]/);
    let col = ["rr","rri","rri_ms","nn","ibi"].reduce((a, k) => a >= 0 ? a : header.indexOf(k), -1);
    if (col < 0) col = 0;
    for (let i = 1; i < lines.length; i++) {
      const v = parseFloat(lines[i].split(/[,;\t]/)[col]);
...
$("rrFile").addEventListener("change",  async () => {
  try { await loadRRFile(); setStatus(`RR cargado (${G.rrBuf.length})`, "live"); } catch (e) { alert(e.message); }
});

async function startSession() {
  G.ppgBuf = []; G.rrBuf = []; G.sigBuf = [];
  G.lastMetrics = null;
  G.totalSec = G.durationMin * 60;
  setQuality(null);
  ...
  try {
    ...
    await countdown("Preparación", meta.guide,
      G.sensor === "polar_h10" ? "Aceptá el permiso Bluetooth." : "No bloquees la pantalla.", 5);
...
async function stopSession(auto = false) {
  ...
  setBtns(false);
  setStatus(auto ? "Completado" : "Detenido", auto ? "live" : "warn");

  if (auto || G.ppgBuf.length >= 30 || G.rrBuf.length >= 12) {
    await computeAndDisplay();
  }
}
...
async function computeAndDisplay() {
  ...
  const payload = {
    sensor_type     : isRR ? G.sensor : "camera_ppg",
  ...
  if (isRR) {
    payload.rri_ms = G.rrBuf.length ? G.rrBuf : G.ppgBuf;
  } else {
    payload.ppg           = G.ppgBuf;
    payload.sampling_rate = isSCG ? SCG_SR : PPG_SR;
  }
  ...
  try {
    ...
    setBtns(false);
    setStatus("Análisis completado", "live");
...
function renderSem(sem, data) {
  const hero = $("semHero");
  hero.className = `sem-hero ${sem.key}`;
  $("semIcon").textContent  = sem.icon;
  $("semLabel").textContent = sem.label;
  $("semDesc").textContent  = sem.description;

  const rmssd = data.rmssd_corr ?? data.rmssd;
  $("semRmssdVal").textContent = rmssd != null ? rmssd.toFixed(1) : "—";

  const norms = data.hba_dashboard?.norms;
  if (norms?.rmssd_low && rmssd != null) {
    const lo = norms.rmssd_low, hi = norms.rmssd_high;
    $("normLow").textContent  = `${lo.toFixed(0)}ms`;
    $("normHigh").textContent = `${hi.toFixed(0)}ms`;
    let pct;
    if (rmssd <= lo)  pct = Math.max(2,  (rmssd / lo) * 35);
    else if (rmssd <= hi) pct = 35 + ((rmssd - lo) / (hi - lo)) * 30;
    else pct = Math.min(98, 65 + ((rmssd - hi) / (hi * 0.5)) * 33);
    $("normMarker").style.left = `${pct}%`;
    $("normBarWrap").style.display = "";
  }
}
...
function renderCargas(cargas) {
  ...
  const clsMap = { bajo: "low", moderado: "mod", alto: "high", "muy alto": "vhigh" };
  $("cargasGrid").innerHTML = items.map(it => {
    const c   = cargas[it.key] || {};
    const v   = c.value != null ? Math.round(c.value) : null;
    const lvl = c.level || "bajo";
    const cls = clsMap[lvl] || "low";
    return `<div class="carga-card ${cls}" style="--pct:${v ?? 0}%">
      <div class="carga-name">${it.label}</div>
      <div class="carga-val">${v ?? "—"}</div>
      <div class="carga-level">${lvl}</div>
    </div>`;
  }).join("");
}
...
function renderMetrics(data) {
  ...
  $("metricsGrid").innerHTML = items.map(i =>
    `<div class="metric-card ${i.c}">
       <div class="metric-key">${i.k}</div>
       <div class="metric-val">${i.v}</div>
       <div class="metric-unit">${i.u}</div>
     </div>`
  ).join("");
}
...
function renderBio(biomarkers) {
  ...
  const stateLabel = { alto:"Alto", medio:"Normal", bajo:"Bajo", informativo:"—", insuficiente:"N/D" };
  $("bioTableWrap").innerHTML = `
    <table class="bio-table">
      <thead><tr>
        <th>Biomarcador</th><th>Valor</th><th>Estado</th><th>Referencia</th>
      </tr></thead>
      <tbody>${biomarkers.map(b => `
        <tr>
          <td class="bio-name">${b.name}</td>
          <td class="bio-val">${f(b.value)} <span class="bio-unit">${b.unit||""}</span></td>
          <td><span class="state-chip ${b.state||"informativo"}">${stateLabel[b.state]||b.state}</span></td>
...
function renderPlan(plan) {
  ...
  $("planList").innerHTML = `<div class="plan-list">${
    plan.map(p => `
      <div class="plan-item">
        <span class="plan-text">${p.item}</span>
        <div class="plan-bar-wrap">
          <div class="plan-bar"><div class="plan-fill" style="width:${p.pct}%"></div></div>
...
async function saveSession() {
  ...
  try {
    ...
    const data = await res.json();
    setStatus(data.ok ? "Guardado ✓" : "Error al guardar", data.ok ? "live" : "err");
